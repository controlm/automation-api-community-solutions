...
@Service
@IntentHandler(intent = Intent.NUMBER_INTENT)
@RequiredArgsConstructor
public class NumberIntent extends WordFillingAbstractRequestProcessor {
    ...
    private Optional<Response> recogniseLetterAndAddToWord(HandlerInput input, User user, Supplier<String> wordGetter, Consumer<String> wordSetter, int position) {
        Optional<String> symbol = findSlotVal(input, "number");
        return symbol.map(s -> {
    ...
    private Optional<String> findSlotVal(HandlerInput input, String slot) {
        try {
            String s = SDKSimplifierUtill.getSlotsByHandlerInput(input).get(slot).getValue();
