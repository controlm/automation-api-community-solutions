...
def lambda_handler(event, context):
    ...
    if eventSource == 'aws:sqs':
        ctmresponse = sqs_event(event)
    elif eventSource == 'aws:sns':
        ctmresponse = sns_event(event)
    elif eventSource == 'aws:s3':
        ctmresponse = s3_event(event)
    else:
...
def sqs_event(event):
    ...
    if ReqType == 'runjob':
        run_ctm_job(ctmtoken, auth.baseurl, CTMServer, Jobname, Folder)
    elif ReqType == 'createjob':
        Template = msgpayload['Request'][0]['RequestAttributes']['Template']
        template_json = get_job_template(Template, auth.bucket, Folder, Jobname)
        create_ctm_jobfile(template_json, ctmtoken, auth.baseurl)
...
def build_auth(CTMEnvironment):
    ...
    baseurl = 'https://' + ctmEndpoint + ':8443/automation-api/'
...
def create_ctm_jobfile(template_json, ctmtoken, baseurl):
    global verbose
    response = 200
    rjoburl = baseurl + 'run'
...
def get_job_template(template, bucket, Folder, Jobname):
    KEY = template + ".template"         # Construct S3 Key
    content_object = s3res.Object(bucket, KEY)
    template_str = content_object.get()['Body'].read().decode('utf-8')
    #template_str = str(json.loads(template_json))
    template_str = template_str.replace('$$FOLDER$$', Folder)
    template_str = template_str.replace('$$JOBPFX$$', Jobname)
