...
def main(argv):
    ...
    if result.status_code == 200:
        print(colors.fg.green+"Job successfully ordered:"+ colors.reset)
        print(result.text)
        sys.exit(0)
    else:
        sys.exit(1)
...
def order_folder(endpoint, token, folder, ctm_server, configuration, config={}, verbose=False):
    ...
    if response.ok:
        if verbose:
            print(colors.fg.green+ "Job ordered"+ colors.reset)
