...
if (($outFile -ne '') -And ($deploymentOutput.ProvisioningState -eq "Succeeded")) {
	$dataFactoryName = $deploymentOutput.Outputs.dataFactoryName.Value
	$dataFactoryNameRecord = "xxxxdataFactoryNamexxxx:$dataFactoryName"
	
	$dataFactoryNameRecord | Add-Content $outFile
}
