#!/bin/bash
#------------------------------------
# This script will add the DR Control-M/Server to the Authorized Servers list at all the agents in array of AGENTS_ARRAY. 
#------------------------------------
...
#NEW_ADDED_AUTHORIZED_CTM_SERVER: the DR Control-M/Server
...
do  
	...
	if [ $? -eq 0 ]; then
		echo "The DR Control-M/Server has been added to Control-M/Agent $AGENT Authorized Servers!"
		echo ""
	else
		echo "Failed to add DR Control-M/Server to Control-M/Agent $AGENT Authorized Servers!"
		echo ""
		exit 1
	fi
done  
