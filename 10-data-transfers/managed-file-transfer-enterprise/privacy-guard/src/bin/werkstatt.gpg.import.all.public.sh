...
# file name: werkstatt.gpg.import.all.public.sh
# purpose : Import every PUBLIC key file staged in $MFTE_GPG_EXCHANGE_DIR
#           (or -d) into the mftgpg keyring in one pass -- the batch
#           counterpart to werkstatt.gpg.import.public.key.sh, for the
#           "onboard 10 customers' public keys at once" case rather than
#           one werkstatt.gpg.import.public.key.sh -f <file> call per key.
#
# scope   : Each file is inspected (never imported blind) before anything
#           happens to it:
#             - files named *.passphrase are skipped outright -- never
#               even handed to gpg, since $MFTE_GPG_EXCHANGE_DIR can
#               legitimately contain one of these if a key was exported
#               with werkstatt.gpg.export.key.sh -W, and a passphrase is
#               not something this script should ever touch.
#             - anything gpg can't read any key material from is skipped
#               (not failed) as "not a key file" -- a stray README or
#               .DS_Store sitting in the same directory shouldn't flip
#               this script's exit code.
#             - a file whose PRIMARY record is "sec" (contains secret key
#               material) is skipped (not imported, not failed) and
#               pointed at werkstatt.gpg.import.all.private.sh instead --
#               this script only ever imports the public half of anything.
#             - a fingerprint already present in the keyring (checked via
#               gpg --list-keys against the file's own fingerprint,
#               BEFORE calling --import) is skipped as "already present",
#               not re-imported. This is what makes the whole directory
#               safe to re-run against on every pass, including ones where
#               most keys are already there.
...
Only imports PUBLIC key material. A file containing a secret key is
skipped and reported, not imported -- use
werkstatt.gpg.import.all.private.sh for those. Safe to re-run: a
fingerprint already in the keyring is skipped, not re-imported.
...
for FILE in "${FILES[@]}"; do
  ...
  if [[ "$HAS_SECRET" == "true" ]]; then
    log_system INFO "skip file=${FILE} reason=secret_key_file fingerprint=${FP}"
    REPORT_LINES+=("SKIP      ${BASE}  reason=secret_key_file  fp=${FP}  (use werkstatt.gpg.import.all.private.sh)")
    SKIPPED_OTHER=$((SKIPPED_OTHER + 1))
    continue
  fi

  if mfte_gpg_lookup_fingerprint "$FP" public >/dev/null 2>&1; then
  ...
  UID_STR="$(mfte_gpg_uid_for_fingerprint "$IMPORTED_FP" public)"
  log_system INFO "imported file=${FILE} fingerprint=${IMPORTED_FP} uid=\"${UID_STR}\""
  REPORT_LINES+=("IMPORTED  ${BASE}  fp=${IMPORTED_FP}  uid=\"${UID_STR}\"")
  IMPORTED=$((IMPORTED + 1))
done
...
GPG batch public key import complete
  directory         : $DIR
