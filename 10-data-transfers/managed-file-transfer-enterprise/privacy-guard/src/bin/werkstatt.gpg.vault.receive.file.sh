...
do_receive() {
  ...
  if ! dec_output_log="$(mfte_gpg_run --batch --yes --pinentry-mode loopback --skip-verify --passphrase-fd 3 --output "$DEC_OUTPUT" --decrypt "$OPERATE_PATH" 3< <(mfte_gpg_vault_get_passphrase "$KEY_FP") 2>&1)"; then
