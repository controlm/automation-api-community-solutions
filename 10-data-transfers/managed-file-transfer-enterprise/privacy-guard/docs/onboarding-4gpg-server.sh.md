...
- **Public key only** → `$MFTE_GPG_ONBOARDING_B2B_DIR/<email>/` (defaults under `/mnt/ftshome/b2bhome/{VFOLDER}/onboarding`, `{VFOLDER}` substituted from the required `-v` flag) for the customer to collect.

`{VFOLDER}` scopes the delivered public key to the *same* per-customer Virtual Folder MFT Enterprise itself provisions during onboarding — a shared top-level folder outside MFTE's own onboarding (e.g. the old default, a literal `secureTransport` directory) is invisible to MFTE's own ACLs, so the customer could never actually browse to their own key there. See [werkstatt.gpg.receive.file.sh](werkstatt.gpg.receive.file.sh.md#virtual-folder-configuration)'s doc for the matching MFT Enterprise UI screenshots — same Virtual Folder, same `onboarding` sub-folder already shown there.
...
```
onboarding-4gpg-server.sh -n "<name>" -m "<email>" -v "<vfolder>" [options]
```
...
| `-v` | **Required.** The customer's MFT Enterprise Virtual Folder name, e.g. `"HighSec"` — substituted into `$MFTE_GPG_ONBOARDING_B2B_DIR`'s `{VFOLDER}` placeholder. Also validated as a plain directory segment (no `/`, no whitespace). Required even when `-B` fully overrides the B2B export path with a literal path that doesn't use `{VFOLDER}` — harmless if unused in that case |
...
| `-B` | Override `$MFTE_GPG_ONBOARDING_B2B_DIR` — `{VFOLDER}` in whatever value ends up here (default or override) is still substituted from `-v` |
| `-q` | Quiet |
| `-h` | Help |

```
onboarding-4gpg-server.sh -n "ACME Finance" -m "finance@acme.example.com" -v "ACMEFinance" -q
```
...
| `2` | Usage error (bad/missing flags, invalid email or vfolder) |
