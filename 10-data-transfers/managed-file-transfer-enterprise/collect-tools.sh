...
collect_project() {
  ...
  # build_package.py is dev-only tooling, never collected.
  local f base
  for f in "$src_root"/*.py "$src_root"/*.sh; do
    base="$(basename "$f")"
    [[ "$base" == "build_package.py" ]] && continue
    stage_file "$project" "$f" "$base"
  done

  # bin/*.sh -> tools/bin/
  for f in "$src_root"/bin/*.sh; do
    stage_file "$project" "$f" "bin/$(basename "$f")"
  done

  # lib/bash/*.sh -> tools/lib/bash/
  for f in "$src_root"/lib/bash/*.sh; do
    stage_file "$project" "$f" "lib/bash/$(basename "$f")"
  done
  ...
  # same exclusion build_package.py already applies.
  ...
  for f in "$src_root"/data/*; do
    [[ -f "$f" ]] || continue
    stage_file "$project" "$f" "data/$(basename "$f")"
  done
}
