...
parser = argparse.ArgumentParser(
    description='Updates Authorized Control-M/Servers list in multiple Agents')
parser.add_argument('-tf', '--tokenfile', dest='tokenfile',
                    type=str, help='File that contains the API Key', required=False)
parser.add_argument('-m', '--mailfile',   dest='mailfile',  type=str,
                    help='Input file containing list of email addresses to add', required=True)
...
if args.tokenfile is not None:
    if not os.path.exists(args.tokenfile):
        print("Unable to read the specified file, %s, or file does not exist." %
              args.tokenfile)
    else:
...
if not os.path.exists(args.mailfile):
    print("Unable to read the specified mailfile, %s, or file does not exist." %
          args.mailfile)
    sys.exit(1)
...
with open(args.mailfile, "r") as f:
    ...
    for line in lines:
        ...
        files = {'userFile': (
            'user.json', '{ "Name" : "'+emailaddress+'", "Roles" : ["Admin"] }')}
        ...
        try:
            ...
            print(
                "Connecting to Automation API REST Server failed with error: " + str(err))
            sys.exit(2)
